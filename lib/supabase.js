...
export async function createPuzzle(userId, puzzle) {
  ...
  if (puzzle.id) row.id = puzzle.id;
  if (puzzle.draft !== undefined) row.draft = puzzle.draft;
...
export async function updatePuzzle(puzzleId, userId, updates) {
  if (!supabase) return false;
  const { error } = await supabase.from("puzzles").update(updates).eq("id", puzzleId).eq("creator_id", userId);
  if (error) console.error("updatePuzzle error:", error);
  return !error;
}

export async function updatePuzzleArchived(puzzleId, userId, archived) {
  if (!supabase) return false;
  const { error } = await supabase.from("puzzles").update({ archived }).eq("id", puzzleId).eq("creator_id", userId);
  if (error) console.error("updatePuzzleArchived error:", error);
  return !error;
}
...
export async function getMyShares(userId) {
  ...
  const { data, error } = await supabase
    .from("shared_puzzles")
    .select("puzzle_id, to_user_id, to:profiles!shared_puzzles_to_user_id_fkey(id, display_name, email)")
    .eq("from_user_id", userId);
  if (error) console.error("getMyShares error:", error);
  return data || [];
}

export async function unsharePuzzle(puzzleId, fromUserId, toUserId) {
  if (!supabase) return false;
  const { error } = await supabase.from("shared_puzzles").delete()
    .eq("puzzle_id", puzzleId).eq("from_user_id", fromUserId).eq("to_user_id", toUserId);
  if (error) console.error("unsharePuzzle error:", error);
  return !error;
}

export async function unsharePuzzleAll(puzzleId, fromUserId) {
  if (!supabase) return false;
  const { error } = await supabase.from("shared_puzzles").delete()
    .eq("puzzle_id", puzzleId).eq("from_user_id", fromUserId);
  if (error) console.error("unsharePuzzleAll error:", error);
  return !error;
}
...
export async function getLeaderboardStats(userIds) {
  if (!supabase) return [];
  const { data } = await supabase.from("results").select("*, puzzles!results_puzzle_id_fkey(creator_id)").in("user_id", userIds);
  return data || [];
}
